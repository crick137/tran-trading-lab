...
export const blogPosts: BlogPost[] = [
    // ─── KO Blog Posts ─────────────────────────────────────────
    {
        slug: "cognitive-bias-12-ko",
        title: "당신의 뇌가 돈을 잃게 만드는 12가지 방법",
        description: "행동경제학이 증명한 12가지 인지편향. 매매할 때 뇌가 당신을 배신하는 순간들. 전부 내가 직접 당해본 것들이다.",
        excerpt: "지표가 문제가 아니었다. 전략이 문제가 아니었다. 내 뇌가 문제였다. 12가지 인지편향, 전부 직접 당해본 것들.",
        image: "/images/blog/cognitive-bias-12.jpg",
        category: "cognitive-bias",
        date: "2026-03-22",
        readingTime: "12분",
        tags: ["인지편향", "행동경제학", "손실회피", "확증편향", "트레이딩 심리", "Kahneman"],
        locale: "ko",
        isFeatured: true,
        content: `## 당신의 뇌가 돈을 잃게 만드는 12가지 방법
...
행동경제학에서는 이걸 "인지편향"이라고 부른다.
쉽게 말하면 — 뇌가 지름길을 택하다가 돈을 날리는 현상.
...
"10만 원 벌었을 때의 기쁨"보다
"10만 원 잃었을 때의 고통"이 2배 크다.

이건 느낌이 아니라 데이터다.
Kahneman & Tversky의 전망이론(1979)이 증명했다.

그래서 우리는
수익은 빨리 먹고, 손절은 계속 미룬다.
"조금만 더 기다리면 본전 올 수 있을 것 같은데..."

그 "것 같은데"가 계좌를 죽인다.

**자가진단:**
손절 버튼 위에 손가락 올려놓고 3초 이상 망설인 적 있으면,
당신의 뇌는 지금 손실회피 모드다.
...
롱 포지션 잡으면 불리쉬 뉴스만 보인다.
숏 잡으면 베어리쉬 뉴스만 보인다.

카드 실험이 유명하다.
A, Q, 4, 7 네 장의 카드.
"모음 뒷면에는 짝수가 있다"를 검증하라고 하면
대부분 A와 4를 뒤집는다.

틀렸다.
정답은 A와 7이다.
가설을 부정할 수 있는 카드를 뒤집어야 하는데,
사람은 본능적으로 긍정하는 증거만 찾는다.

매매에서 이게 뭘 의미하냐면.
내가 "BTC 10만 간다"고 믿는 순간,
내 뇌는 10만 가는 근거만 수집하기 시작한다.

반대 시나리오?
무의식적으로 무시한다.

**자가진단:**
포지션 잡은 후에 반대 의견을 일부러 찾아본 적이 있는가?
없으면 확증편향 100%.
...
그 전에 자기 핸드폰 번호 뒷자리를 적게 했다.
뒷자리 7000 이상인 그룹은 평균 8000명이라고 답했고,
3000 이하인 그룹은 평균 4000명이라고 답했다.

핸드폰 번호가 런던 의사 수와 무슨 상관인가?
상관없다.
근데 뇌는 처음 본 숫자에 닻을 내린다.
...
시장은 내가 얼마에 샀는지 모른다.
내 매수가는 시장에 아무 의미가 없다.
그런데 내 뇌는 그 숫자를 절대 놓지 않는다.
...
설문조사 결과.
펀드매니저의 74%가 자기 실력이 평균 이상이라고 답했다.

수학적으로 불가능하다.

경마 실험도 있다.
5개 변수로 말을 고를 때와 40개 변수로 고를 때,
적중률은 똑같았다.
하지만 40개 변수를 본 그룹의 "확신"은 크게 올라갔다.

정보가 많아지면 실력이 아니라 자신감이 올라간다.
그리고 그 자신감이 포지션 사이즈를 키운다.

이게 과신이다.
"이번엔 확실해"라는 말이 나올 때마다
이 실험을 떠올려야 한다.
...
1980년부터 2002년까지 미국 3만 개 펀드를 분석한 연구가 있다.
결과: 성과가 가장 나쁜 펀드의 공통점은
"이기는 포지션을 빨리 팔고, 지는 포지션을 오래 들고 가는" 펀드였다.

왜 이러는 걸까?
손실회피(①번) 때문이다.
수익은 확정하고 싶고 — 잃을까 봐.
손실은 확정하기 싫고 — 인정하기 싫으니까.

이건 감정이 아니라 뇌의 구조다.

**자가진단:**
지금 포트폴리오에서 수익 종목과 손실 종목 중
어느 쪽을 먼저 정리하고 싶은지 생각해보라.
수익 쪽이면, 처분효과에 걸린 거다.
...
선거 실험.
모든 후보 정보를 공개했을 때, 83%가 1번을 선택했다.
하지만 "다수가 2번을 지지한다"는 정보를 추가하자,
1번 지지율이 18%로 떨어졌다.

뇌과학 연구에 따르면
사회적 고통(소외감)을 처리하는 뇌 영역과
물리적 고통을 처리하는 뇌 영역이 같다.

무리에서 벗어나면, 뇌는 문자 그대로 아프다.

그래서 모두가 사는데 나만 안 사면 불안하고,
모두가 파는데 나만 안 팔면 불안하다.
...
1913년. 몬테카를로 카지노.
룰렛에서 검은색이 연속 26번 나왔다.
확률? 약 6억 7천만 분의 1.

그런데 사람들은 15번쯤부터 미친 듯이 빨간색에 걸었다.
"이제 빨간색 나올 때 됐지."
수백만 프랑이 증발했다.

매매에서도 똑같다.
3번 연속 손절했으면,
"다음 건 이겨야 정상이지" 라고 느낀다.

각 매매는 독립 사건이다.
이전 결과가 다음 결과에 영향을 주지 않는다.

근데 뇌는 패턴이 없는 곳에서도 패턴을 찾는다.
그게 인간이다.
...
스텔스 전투기 실험.
"이미 900만 달러 투입. 나머지 100만 달러로 완성 가능."
80%가 "투자 계속"을 선택했다.
...
"이미 30% 물려있는데 여기서 팔면 진짜 손해잖아."
이 생각이 들 때, 스텔스 전투기를 떠올려라.

시장은 당신이 얼마를 투입했는지 모른다.
매몰비용은 이미 사라진 돈이다.
의사결정에 포함시키면 안 된다.
...
매매에서 이건 독이다.
원칙 무시하고 들어갔는데 운 좋게 이기면 — "역시 내 감이 맞았어."
원칙 지키고 들어갔는데 손절 당하면 — "이 전략 별로네."
...
배부를 때 다이어트 계획 세우기는 쉽다.
배고플 때 지키기는 불가능하다.

매매도 마찬가지다.
차트 보면서 "여기서 사고, 여기서 손절하자"는 쉽다.
실제로 -15% 찍고 손절 버튼을 누르는 건 완전히 다른 일이다.

동전 게임 실험이 있다.
50% 확률로 2.5배 수익 / 50% 확률로 1배 손실.
기대값은 양수다. 매번 거는 게 맞다.

정상인: 58%만 베팅.
공포를 느끼지 못하는 뇌 손상 환자: 84% 베팅.
...
축구 페널티킥 데이터.
킥의 1/3은 중앙으로 간다.
그런데 골키퍼의 94%가 좌우로 몸을 날린다.

왜?
가만히 서 있으면 "노력 안 한 것처럼 보이니까."
...
"이 전략으로 10배 벌었습니다."
"이 코인 초기에 사서 인생 바뀌었습니다."

우리가 보는 건 살아남은 사람의 이야기다.
같은 전략으로 계좌 날린 99명은 글을 안 쓴다.

2차 대전 폭격기 사례가 유명하다.
귀환한 비행기의 피탄 부위를 강화하려 했는데,
통계학자가 말했다. "돌아오지 못한 비행기를 봐야 한다."

소셜 미디어의 수익 인증은 귀환한 비행기다.
당신이 봐야 할 건 돌아오지 못한 계좌다.

**자가진단:**
트위터에서 "수익 인증"을 볼 때,
"이 사람과 같은 전략을 쓰고 망한 사람은 몇 명일까?"를
한 번이라도 생각해봤는가?

---

이 12가지를 읽고 "나는 안 그런데?"라고 생각했다면,
그게 **13번째 편향**이다.
