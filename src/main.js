// ===== TRAN TRADING LAB — src/main.js (stable router + pages + bg FX) =====
import './styles/global.css';
import './components/x-tv-chart.js';

// JS 已加载，移除 no-js（如果你在 <html class="no-js"> 做了 CSS Fallback）
...
function matchRoute() {
  ...
  // 常规映射 + HTML 可用性校验
  let id = routes[h] || 'home';
  const available = [...document.querySelectorAll('section[data-route]')].map(s => s.dataset.route);
  if (!available.includes(id)) id = available.includes('home') ? 'home' : (available[0] || 'home');
  return { id };
}
...
// ---------------------- Background K-lines FX -------------------------
(() => {
  if (window.__bgFxInit) return; window.__bgFxInit = true;
...
function ensureKLControls() {
  ...
  if (!host || document.getElementById('kl-controls')) return;
...
function boot() {
  ...
  if (m.id === 'daily-brief') renderDailyBriefList();
  if (m.id === 'daily-brief-detail') renderDailyBriefDetail(m.slug);
  if (m.id === 'trade-journal') {
    loadAnalysesList().then(() => {
      ...
      renderAnalysisDetailBySlug(currentSlugFromQuery());
    });
  }
  if (m.id === 'knowledge-lab') renderKnowledgeLab();
}
...
window.addEventListener('hashchange', async () => {
  try {
    ...
    console.error('[router] hashchange error', e);
    show('home');
  }
});

window.addEventListener('DOMContentLoaded', () => {
  try { boot(); } catch (e) { console.error('[router] boot error', e); show('home'); }
});
...
function localizeKnowledgeLabKO() {
  ...
  if (!sec) return;
  const h1 = sec.querySelector('.hero h1'); if (h1) h1.textContent = '체계적 외환 지식 · 로드맵';
  ...
  if (progressTitle && /进度|总体|整体|전체 진행도/.test(progressTitle.textContent)) progressTitle.textContent = '전체 진행도';

  const resetBtn = sec.querySelector('#kl-reset, .progress-reset, .kl-reset'); if (resetBtn) resetBtn.textContent = '진행도 초기화';
  const q = document.getElementById('kl-q'); if (q) q.placeholder = '강의 검색…';
...
window.addEventListener('hashchange', () => { if (location.hash === '#/knowledge-lab') localizeKnowledgeLabKO(); });
document.addEventListener('DOMContentLoaded', () => { if (!location.hash || location.hash === '#/knowledge-lab') localizeKnowledgeLabKO(); });
