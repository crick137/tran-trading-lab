// === api/app.js (hotfix 索引稳定 200 + []、删除索引修复、og:image) ===
...
function withHeaders(init={}){ const h=new Headers(init.headers||{}); if(ENABLE_CORS){h.set('access-control-allow-origin','*');h.set('access-control-allow-methods','GET,POST,PUT,DELETE,OPTIONS');h.set('access-control-allow-headers','content-type,authorization,cookie');} return {...init,headers:h};}
function getHeader(req,name){ const h=req.headers||{}; const k=String(name).toLowerCase(); if(typeof h.get==='function') return h.get(name); return h[k]||h[name]||null; }
function getURL(req){ try{ if(req.url && String(req.url).startsWith('http')) return new URL(req.url); const proto=getHeader(req,'x-forwarded-proto')||'https'; const host=getHeader(req,'x-forwarded-host')||getHeader(req,'host')||'localhost'; const path=req.url||'/'; return new URL(`${proto}://${host}${path.startsWith('/')?'':'/'}${path}`);}catch{ return new URL('https://localhost/api/ping');}}
async function readBody(req){ try{ const ct=(getHeader(req,'content-type')||'').toLowerCase(); if(typeof req.json==='function' && ct.includes('application/json')) return await req.json(); if(typeof req.text==='function'){ const t=await req.text(); return t?JSON.parse(t):{}; } let data=''; if(req.readable){ for await(const ch of req) data+=ch; return data?JSON.parse(data):{};} return {};}catch{return {};}}
...
async function upsertIndex(prefix,key){
  ...
  try{ await writeJSON(INDEX,arr,{timeoutMs:2000,retries:1,retryDelayMs:200}); }catch(e){ console.warn('[INDEX] upsertIndex fast-path failed',e?.message||e); }
}
...
function requireAuthIfConfigured(req){
  ...
  try{ const cookie=getHeader(req,'cookie')||''; if(cookie && /(?:^|;\s*)tran_admin=ok(?:;|$)/.test(cookie)) return null; }catch{}
  return _requireAuth(req);
}

/* --------- 显式稳定索引：永远 200 + []（修复前端崩溃） --------- */
...
async function genericHandler(req, pathname, PREFIX){
  try{
    ...
    if (req.method==='GET'){
      ...
      const cands=Array.from(new Set([ slug,
    ...
    if (['PUT','POST'].includes(req.method)){
      ...
      try{
        ...
        try{
          if(PREFIX==='analyses'){
            const meta={ slug, title:body?.title||slug, symbol:body?.symbol||body?.chart?.symbol||'',
              tf:body?.tf||'', date:body?.date||'', tags:Array.isArray(body?.tags)?body.tags:[], bias:body?.bias||'' };
      ...
      }catch(e){ return err(e?.message||'WRITE_FAILED',500); }
    }
...
async function handleResearch(req, pathname){
  ...
  if (/^\/api\/research\/articles(?:\.json)?$/.test(p)) {
    // 若有静态索引文件就读，否则返回 []
    try{ return ok(await readJSONViaFetch('research/articles/index.json')); }catch{ return ok([]); }
  }
...
async function handleOgImage(urlObj){
  ...
  }catch(e){ console.warn('[og-image] fail',e?.message||e); return ok({ image:null }); }
}

/* ---------- 响应适配 ---------- */
async function sendNodeResponse(res,out){
  if(out && typeof out==='object' && typeof out.text==='function' && out.headers){
    const status=out.status||200; const headersObj={}; try{ for(const [k,v] of out.headers.entries()) headersObj[k]=v; }catch{}
    const body=await out.text(); res.writeHead(status,headersObj); res.end(body); return;
  }
...
export default async function handler(req,res){
  try{
    ...
    let out;
    if (pathname==='/api/daily-brief/index.json' && req.method==='GET') out = await stableIndex('daily-brief');          // ✅ 显式稳定
    else if (pathname==='/api/research/articles/index.json' && req.method==='GET') out = await stableIndex('research/articles'); // ✅ 显式稳定
      ...
      const unauthorized=requireAuthIfConfigured(req); if(unauthorized) out=unauthorized; else{
        ...
        if(!id) out=err('MISSING_ID',400);
          ...
          try{ await writeJSON(`market-news/${id}.json`,body); }catch(e){ out=err('WRITE_FAILED',500); }
          if(!out){ try{ await upsertIndex('market-news',id);}catch(e){} out=ok({ saved:true, id }); }
        }
      }
    }
      ...
      if(req.method==='GET') out=await handleResearch(req,pathname); else out=await handleResearchSyllabusWrite(req);
    }
